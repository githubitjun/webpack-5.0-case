    ...
    <div>
        ...
        <el-upload
            ...
            :data="{
                ...
                access_token: JSON.parse(getSessionStorage('userInfo')).session_id,
                org_code: JSON.parse(getSessionStorage('userInfo')).org_code,
            }"
            :multiple="true"
...
/**
     ...
     *      fileType：允许上传的文件类型
     *      fileSize：总文件最大几M
     *      singleFileSize： 单个文件最大几M
     *      isShowTips： 是否显示tips
     *      buttonType： 按钮类型
     *      needImagePaste：是否需要粘贴图片功能
     *      onlyImagePaste：是否只有粘贴图片的功能
...
export default {
    ...
    props: {
        ...
        showFileList: { // 是否显示已上传文件列表 默认true
            type: Boolean,
            default: true,
        },
        multiple: { // 是否支持多选文件
    ...
    methods: {
        getSessionStorage: storage.getSessionStorage,
        // 外部调用的方法
        ...
        beforeUpload (file) {
            ...
            if (this.fileType.length && !this.fileType.includes(ext)) {
                // console.log('支持的格式：', this.fileType);
